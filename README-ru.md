...
Утверждения с `"tier": "bonus"` оцениваются и выводятся (`✅ [bonus]` пройдено, `➖ [bonus]` нет), но не влияют на результат кейса. Используйте bonus для проверок стиля и формулировок ответа (они колеблются в зависимости от relay-бэкенда); проверки эффектов в файловой системе оставляйте обязательными.
