...
| T3 | Точность срабатывания skill (один вызов `claude -p`); `skip` при отсутствии API-учётных данных |
| T5 | Поведенческий набор оценок (выполняет `evals/evals.json` в контейнере; `skip` при `disable_t5: true` или отсутствии API-учётных данных) |

### Учётные данные контейнера (T3/T5)

T3/T5 запускают `claude` внутри контейнера, для чего нужен API-ключ. Порядок разрешения:

1. `~/.claude/looper/credentials.env` (рекомендуется — хост с подпиской OAuth остаётся нетронутым; chmod 600):
   ```
   ANTHROPIC_API_KEY=sk-...
   ANTHROPIC_BASE_URL=https://...   # необязательно, для relay
   ```
2. Блок `env` в файле хоста `~/.claude/settings.json`

Если ключа нет ни в одном из источников, T3/T5 помечаются как `skip` — пробел окружения, а не сбой плагина.
...
Утверждения в `evals.json` по умолчанию — обычные строки: каждое строковое утверждение должно пройти, чтобы кейс был засчитан. Утверждение можно записать и как объект с уровнем:
...
Утверждения с `"tier": "bonus"` оцениваются и выводятся (`✅ [bonus]` пройдено, `➖ [bonus]` нет), но не влияют на результат кейса. Используйте bonus для проверок стиля и формулировок ответа (они колеблются в зависимости от relay-бэкенда); проверки эффектов в файловой системе оставляйте обязательными. Кейс с непройденными обязательными утверждениями перезапускается один раз в чистом каталоге; кейс засчитывается, если прошла любая из двух попыток.
